...
public:
    ...
    { 
        ...
        //-------------------------- THREAD ---------------------------------------------------------------
        balThread  = new comStepperThread(rate,dd_torso,dd_rightLeg,dd_leftLeg,robot_name,local_name,wbs_name,\
                                            display,noSens,ankles_sens,springs,torso,verbose,Kp_zmp_x,Kd_zmp_x,Kp_zmp_y,\
                                            Kd_zmp_y,Kp_theta,Kd_theta,Kp_phi,Kd_phi,comPosInputPortName,comJacInputPortName);
    ...
    bool updateModule()
    {
        ...
        fprintf(stderr,"Error in position and orientation: ep: %s     ;  eo: %s    \n",balThread->ep.getCol(0).toString().c_str(),balThread->eo.getCol(0).toString().c_str());
        fprintf(stderr,"Error COM projection:               e: %s                  \n",(balThread->pi_a_d - balThread->pi_a).getCol(0).toString().c_str());
        return true;
    }
};
