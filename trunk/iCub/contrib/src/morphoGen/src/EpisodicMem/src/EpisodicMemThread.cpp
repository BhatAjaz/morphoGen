...
bool EpisodicMemThread::threadInit() {
	...
	N=1000;
	//MotCom.open("/v");
	Network::connect("/objects", "/world/analysis:i"); 
	Network::connect("/Strategy:o", "/plotter");  //check this
    return true;
}
		...
		{            
		   for (int m =0; m<42; m++)
    		...
    		Bottle* ObjIdd = WorldSnap.read(true);
			  // This initialization can from from bottom up perception
			int xid=0;
			for(int i=0;i<5;i++)
				{
				  xid = ObjIdd->get(i).asInt();
				  if(xid != 50){
                  OHub[xid]=1;
				  }
				  cout << "Objject ID received" << xid << endl;
				}
      	}
		for (int m =0; m<42; m++)
		   {
