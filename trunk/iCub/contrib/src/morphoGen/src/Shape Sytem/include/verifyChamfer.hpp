                       ...
                       double k_linear, double b_linear)
{
	...
	if (be_ori_pi)
		...
		T = PI_2;
  	int n_el = edge_temp.cols*edge_temp.rows;
	int n_edge = 0;
    int n_match = 0;
	float diff;
...
/*	for (int i=0;i<n_el;i++)
	{
		for (int j=0;j<
	{
		if (edge_temp.at<unsigned char>(i)>0)//edge 0, 1
		{
			n_edge++;
			if (dist_test.at<float>(i)<th_dist)
			{
				diff = fabs(orient_test.at<float>(i)-orient_temp.at<float>(i));
				if ((diff<th_orient)||(T-diff<th_orient))//circular distance
					n_match++;
			}
		}
...
{
	int x0,y0,w0,h0;
	x0 = (int)round(lu_pt.x/s);
	y0 = (int)round(lu_pt.y*asp/s);
	w0 = (int)round(norm_w/s);
	h0 = (int)round(norm_h*asp/s);
