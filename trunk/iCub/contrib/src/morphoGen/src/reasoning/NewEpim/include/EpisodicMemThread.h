...
private:
	...
	yarp::os::BufferedPort<yarp::os::Bottle >  HubObject,HubBody;
    ...
    std::string pathPrefix,fileName; // the path for input output files
		...
		int WBody2Epim[1000][42];
	    int WBody2EpimT[42][1000];
		...
		int Action[9];
		int IndexM[5],PEnd;
		int NRelPast[15][1000],NRelEp,noverl;
		int NowObAcSeq[15][1000];
		double HubTopDown[5][42],SumTopDown,SumTopDownB;
...
//    void setColorPath(std::string inp) { colorMapPath = inp; };
  ...
  //  void setModelPath(std::string inp) { modelPath = inp; };
    	...
    	void MemControl(int HubQuery, int GoalContext);

        int TDMemCompHub(int Nrelev);

		int TDMemCompBodyHub(int Nrelevant);
		...
		int RememberPast(int HubID, int GoalRoot);
