...
for i= 1:totalnumberofsteps
    ...
    elseif i==totalnumberofsteps
        if LeftFootMoving(i)% Right foot stance
            ...
            % DS
            zmpy( round(TDS_start(i)/T) : round(TDS_end(i)/T) )  = interp1( [TDS_start(i) TDS_end(i)], [ YGlobalFootL(i+1), 0.5*(YGlobalFootR(end)+YGlobalFootL(end)) - 0.06 ]  ,[TDS_start(i) : T: TDS_end(i)] );
