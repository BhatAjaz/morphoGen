...
private:
    ...
    int sampleCount;
    bool checkConstants(const DMPstructure &y);
public:
    /**
...
//     DmpGPR();
     ...
     */
    bool feedSample(const yarp::sig::Vector &x, const DMPstructure &y);
    ...
    */
     ...
     */
    bool inference(const std::vector <yarp::sig::Vector> & x, const std::vector<DMPstructure> & y, bool optimizeHyperparameters=false);
     ...
     */
    DMPstructure generalize(const yarp::sig::Vector & xs);
     ...
     * \return list of DMP with predicted parameters (weights & tau) //should add goal as well...?
     */
    std::vector<DMPstructure> generalize(const std::vector <yarp::sig::Vector> & xs);
