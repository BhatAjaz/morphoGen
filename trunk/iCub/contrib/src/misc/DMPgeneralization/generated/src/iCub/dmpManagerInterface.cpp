...
public:
  ...
  virtual bool read(yarp::os::ConnectionReader& connection) {
    yarp::os::idl::WireReader reader(connection);
    if (!reader.readListReturn()) return false;
    if (!reader.readString(_return)) return false;
    return true;
  }
};
...
bool dmpManagerInterface::read(yarp::os::ConnectionReader& connection) {
  ...
  while (!reader.isError()) {    // TODO: use quick lookup, this is just a test
    if (tag == "train") {
      ...
      std::string tool;
      std::string hand;
      if (!reader.readString(action)) return false;
      if (!reader.readString(target)) return false;
      if (!reader.readString(tool)) return false;
      if (!reader.readString(hand)) return false;
      bool _return;
