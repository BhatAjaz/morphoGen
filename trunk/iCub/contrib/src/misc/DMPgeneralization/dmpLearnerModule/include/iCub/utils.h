...
public:
    ...
    bool addActionDMP(int32_t id, DMPstructure &dmp);
    std::vector<int32_t> get_all_actions_with_traj_endeff();
    bool get_information_for(int32_t id);
   // yarp::sig::Matrix get_trajectory(int32_t id); //ATTN! need to retrieve also target position and object name... maybe use just get_DMP
    //DMPstructure get_DMP(int32_t id);
