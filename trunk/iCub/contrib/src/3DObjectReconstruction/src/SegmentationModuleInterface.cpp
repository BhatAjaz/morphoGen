...
public:
  yarp::sig::Pixel objCenter;
  std::vector<yarp::sig::Pixel>  _return;
  virtual bool write(yarp::os::ConnectionWriter& connection) {
    yarp::os::idl::WireWriter writer(connection);
...
std::vector<yarp::sig::Pixel>  SegmentationModuleInterface::get_component_around(const yarp::sig::Pixel& objCenter) {
  std::vector<yarp::sig::Pixel>  _return;
    ...
    if (tag == "get_component_around") {
      ...
      if (!writer.isNull()) {
        if (!writer.writeListHeader(1)) return false;
        {
          ...
          std::vector<yarp::sig::Pixel> ::iterator _iter5;
