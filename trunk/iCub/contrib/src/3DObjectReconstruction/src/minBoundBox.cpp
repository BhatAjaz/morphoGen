...
MinBoundBox::MinBoundBox(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud)
{
    this->cloud=cloud;
}

void MinBoundBox::retrieveMinimumBox(Matrix &cornerpoints, Matrix &rotmat)
{
    ...
    cornerpoints=cornerpointsTmp*(rotmat.transposed());
}

void MinBoundBox::findRotation(const Matrix &xyz_i, Matrix &rot2)
{
...
void MinBoundBox::findRotation2D(const Matrix &xy, Matrix &rot2)
{
...
void MinBoundBox::assignCorners(const Matrix &minmax, Matrix &cornerpoints)
{
...
void MinBoundBox::buildRotMat2(const double &theta,yarp::sig::Matrix &rot)
{
    rot.resize(2,2);
    rot(0,0)=cos(theta);
    rot(0,1)=sin(theta);
...
void MinBoundBox::buildRotMat3(const double &alpha,const double &beta,const double &gamma,yarp::sig::Matrix &rot)
{
    rot.resize(3,3);
...
void MinBoundBox::eulerAngles(const std::vector<yarp::sig::Vector> &edges1,const std::vector<yarp::sig::Vector> &edges2,const std::vector<yarp::sig::Vector> &crossProduct,yarp::sig::Vector &alpha,yarp::sig::Vector &beta,yarp::sig::Vector &gamma)
{
    double singamma;
...
void MinBoundBox::retrieveEdges(const yarp::sig::Vector &vx, const yarp::sig::Vector &vy, const yarp::sig::Vector &vz, std::vector<yarp::sig::Vector> &edges)
{
...
void MinBoundBox::retrieveEdges2(const yarp::sig::Vector &vx, const yarp::sig::Vector &vy, const yarp::sig::Vector &vz, const std::vector<yarp::sig::Vector> &edges1, std::vector<yarp::sig::Vector> &edges)
{
