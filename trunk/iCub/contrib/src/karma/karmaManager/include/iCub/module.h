...
protected:
   ...
   // void                        acquireImage(const bool rtlocalization=false);
    ...
    int                         executeToolSearchOnLoc( string objName );
    yarp::os::Bottle            executeBlobRecog( string objName );

    bool                        executeCloseHand(int ARM);
    bool                        executeDropAway(int ARM);
    bool                        executeGiveAction(int ARM);
    bool                        executeSpeech( string speech );
    double                      executeVirtualDraw(blobsData &blobsDetails);
    double                      executeToolDrawNear(blobsData &blobsDetails);
    int                         executeToolAttach(yarp::sig::Vector tool);
    yarp::os::Bottle            executeKarmaOptimize( yarp::sig::Vector tool, string objName);
    ...
    bool                        executePCLGrasp( string objName );
