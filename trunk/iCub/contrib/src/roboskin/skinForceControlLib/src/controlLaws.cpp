...
Status JPositionControlLaw::abstractCompute(const CtrlRef &ref, const RobotStatus &s, Vector &out, double &feasibility)
{
    ...
    Vector ddq_d = CTRL_DEG2RAD * (kp*(ref.q-s.q) + kd*(ref.dq-s.dq) + ref.ddq + pid->compute(ref.q, s.q));
    // inverse dynamics
    limb->setAng(CTRL_DEG2RAD*s.q);
...
#define _DEBUG
    ...
    printf("tau_d: %s\n", (out-limb->computeCcGravityTorques(s.d2p0, CTRL_DEG2RAD*s.q, CTRL_DEG2RAD*s.dq)).subVector(3,7).toString(2).c_str());
#endif
