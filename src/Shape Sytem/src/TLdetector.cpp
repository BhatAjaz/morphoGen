...
void CTLdetector::initiate()
{
	...
	obj_para.n_cls = 8;
	obj_para.all_cls = new string[obj_para.n_cls];
	obj_para.all_cls[0] = "block";
	obj_para.all_cls[1] = "eye";
	obj_para.all_cls[2] = "screw";
	obj_para.all_cls[3] = "driver";
	obj_para.all_cls[4] = "bridge";
	obj_para.all_cls[5] = "whiteblock";
	obj_para.all_cls[6] = "cup";
	obj_para.all_cls[7] = "lid";
 ...
 *
 * INPUT:
 *   tr_file      -- the template data file (*.yml or *.bin)
 *   which_method -- Two methods are available:
 *                   TABLE_PLAIN   0
 *                   TABLE_GROUPPT 1
 * OutPUT:
 *  if it goes correctly, true.
 */
bool CTLdetector::train(string tr_file,int which_method)
{
	//////////////////////////////////////////////////////
	//step1: load training data
	//////////////////////////////////////////////////////
	printf("//step1: load training data\n");
	if (tr_file.substr(tr_file.find_last_of(".") + 1) == "bin") {
		if (!loadTrainFileBin(tr_file))
			return false;
	}
	else if (tr_file.substr(tr_file.find_last_of(".")+ 1) == "yml"){
		if (!loadTrainFileYml(tr_file))
			return false;
	}
	else{
		cout<<"ERROR: only .bin and .yml are surported for the training file!"<<endl;
		return false;
	}
...
void CTLdetector::showDetObjs(Mat im,Scalar box_color,Scalar edge_color,int w_show_wind)
{
	...
	if ((w_show_wind<1)||(w_show_wind==im.cols)) //show the original image size
	{
		for (int i=0;i<vec_detobj.size();i++)
		{
			...
			// then put the text
			std::sprintf(buf,"%s:%.2f",obj_para.all_cls[vec_detobj[i].id_label].c_str(),vec_detobj[i].match_score);
			text = buf;
	...
	else //resize the image
	{
		...
		for (int i=0;i<vec_detobj.size();i++)
		{
			...
			x0 = (int)round(rt0.x*s);
			y0 = (int)round(rt0.y*s);
			w0 = (int)round(rt0.width*s);
			h0 = (int)round(rt0.height*s);
			Rect rt(Point(x0,y0),Size(w0, h0));
			x0 = (int)round(rt_tight0.x*s);
			y0 = (int)round(rt_tight0.y*s);
			w0 = (int)round(rt_tight0.width*s);
			h0 = (int)round(rt_tight0.height*s);
  ...
  * INPUT:
  *    im             -- original image matrix
  *    which_method   -- Two methods are available: TABLE_PLAIN (0), TABLE_GROUPPT (1)
  * OUTPUT:
  ...
  */
bool CTLdetector::detect(Mat im, int which_method)
{
	if (BEPRINT)
	...
	int n_te,i_s, i_asp,n_count_this_s;
	float s,asp;
	double start;
		...
		for (i_asp=test_para.min_aspect;i_asp<=test_para.max_aspect;i_asp++)///////////////////
		{
			...
			if (which_method==TABLE_GROUPPT)
				vec_cur=NNVerifyChamfer(mat_matches, grouppt_para.nmin_matches,mat_te_dis_im, mat_te_ori_im,
					...
					obj_para.n_cls, time_findBestCandi,time_matchRatioDisOri,n_count_run_match);
			else
				vec_cur=NNVerifyChamfer(mat_matches, test_para.nmin_matches,mat_te_dis_im, mat_te_ori_im,
						s, asp,	vec_lu_pts, vec_mat_tr_edge, vec_mat_tr_ori, mat_tr_rect_tight, vec_tr_id_labels,
					be_ori_pi, chamfer_para,
					obj_para.n_cls, time_findBestCandi,time_matchRatioDisOri,n_count_run_match);
			for (int p=0;p<vec_cur.size();p++)
...
int CTLdetector::refineObj(vector<DetObj> & vec_candi_detobj)
{
	...
	// remove
	/////////////////////////////
	for (i=0;i<vec_candi_detobj.size();i++){
		vec_candi_detobj[i].mat_edge_NN_tr.release();
	}
	vec_candi_detobj.clear();
	mat_overlap.release();
	delete []p_area;
